...
<body style="background-image:url('images/about-bg.jpg');background-repeat:repeat;background-position: center;background-size:auto;background-attachment: fixed;">
	...
	<tr>
		...
		<th style='color:white'><center>Message</center></th>
...
<?php
	include 'connection.php';
	$sel=mysqli_query($conn,"SELECT * FROM callback");
	if(mysqli_num_rows($sel)>0)
	{
		while ($fe=mysqli_fetch_assoc($sel)) 
		{
			...
			<tr>
				...
				<td style='color:white'><center>".$fe['email']."</center></td>
				<td style='color:white'><center>".$fe['phone']."</center></td>
				<td style='color:white'><center>".$fe['message']."</center></td>
				<td style='color:white'><center>
				<button class='btn btn-light ml-3 ubb'>Action</button>
				<button class='btn btn-info ml-3 pbb'>React</button>
				<button class='btn btn-warning ml-3 dbb'>Read</button>
				</td>
			</tr> ";
		}
	}
...
<script>
	$(document).ready(function(){
		...
		$("body").on("click",".pbb",function(){
			$(this).closest("tr").css("background-color","skyblue");
		});
